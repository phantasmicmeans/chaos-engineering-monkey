...
public interface ClientEureka {
    ...
    @GetMapping(value = "v1/sub-problem/eureka/actuator/info")
    ...
    @GetMapping(value = "v1/sub-problem/eureka/applications/info") //(o) 유레카에 등록된 모든 어플리케이션 - 명세는 간단히
    @ApiOperation(value = "eureka에 등록된 모든 application의 간단 정보 조회", protocols = "http")
    ResponseEntity getApplicationsFromEureka();

    @GetMapping(value = "v1/sub-problem/eureka/application/{applicationName}/info", produces = "application/json") // (o) applications들 중 이름으로 application 명세, 간단히
    ...
    @GetMapping(value = "v1/sub-problem/eureka/applications/{applicationName}/info/detail", produces = "application/json") //eureka instanceId로 명세 찾기
    ...
    @GetMapping(value = "v1/sub-problem/eureka/regions/info", produces = "application/json")
    @ApiOperation(value = "eureka로 부터 사용중인 모든 region 조회", protocols = "http")
    ResponseEntity getAnyRegionsFromEureka();
    // 모든 리전 명세

    @GetMapping(value = "v1/sub-problem/eureka/regions/{regions}/applications/info", produces = "application/json")
