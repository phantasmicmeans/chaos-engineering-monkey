...
public interface ClientEureka {
    ...
    @GetMapping(value = "v1/problem/eureka/actuator/info")
    ...
    @GetMapping(value = "v1/problem/eureka/applications/info") //(o) 유레카에 등록된 모든 어플리케이션 - 명세는 간단히
    @ApiOperation(value = "eureka에 등록된 모든 application의 간단 정보 조회", protocols = "http")
    ResponseEntity getApplicationsFromEureka();

    @GetMapping(value = "v1/problem/eureka/application/{applicationName}/info", produces = "application/json") // (o) applications들 중 이름으로 application 명세, 간단히
    ...
    @GetMapping(value = "v1/problem/eureka/applications/{applicationName}/info/detail", produces = "application/json") //eureka instanceId로 명세 찾기
    ...
    @GetMapping(value = "v1/problem/eureka/regions/info", produces = "application/json")
